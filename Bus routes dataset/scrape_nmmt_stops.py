...
INPUT_FILE = os.path.join(script_dir, "nmmt_lines_list.json")
OUTPUT_FILE = os.path.join(script_dir, "navi_mumbai_nmmt_bus_routes.json")
...
async def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: {INPUT_FILE} not found. Please run collect_nmmt_lines.py first.")
        sys.exit(1)
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        all_routes = json.load(f)
        
    # Filter to only keep NMMT agency lines (agency ID 1016766)
    all_routes = [r for r in all_routes if "-1016766-" in r["url"]]
    print(f"Filtered to NMMT agency lines only. Loaded {len(all_routes)} NMMT routes to scrape.")
