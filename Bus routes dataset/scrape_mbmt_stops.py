...
INPUT_FILE = os.path.join(script_dir, "mbmt_lines_list.json")
OUTPUT_FILE = os.path.join(script_dir, "mira_bhayandar_mbmt_bus_routes.json")
...
async def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: {INPUT_FILE} not found. Please run collect_mbmt_lines.py first.")
        sys.exit(1)
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        all_routes = json.load(f)
        
    # Filter to only keep MBMT agency lines (agency ID 983982)
    all_routes = [r for r in all_routes if "-983982-" in r["url"]]
    print(f"Filtered to MBMT agency lines only. Loaded {len(all_routes)} MBMT routes to scrape.")
