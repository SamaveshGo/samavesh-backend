...
INPUT_FILE = os.path.join(script_dir, "lines_list.json")
OUTPUT_FILE = os.path.join(script_dir, "mumbai_best_bus_routes.json")
...
async def scrape_route(context, route_info, sem, progress_data, progress_lock):
    ...
    async with sem:
        ...
        # Resource blocker: block ads, images, media, and fonts to speed up loading
        ...
        for attempt in range(retries):
            try:
                ...
                # Wait for the stops to render in the DOM (h3 elements inside .stops-list)
                await page.wait_for_selector(".stops-list h3", timeout=20000)
                
                # Extract all stop titles from h3 tags
                ...
                # print(f"[{route_num}] Error on attempt {attempt+1}: {e}")
                await asyncio.sleep(1)
...
async def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: {INPUT_FILE} not found. Please run collect_lines.py first.")
        sys.exit(1)
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        all_routes = json.load(f)
        
    print(f"Loaded {len(all_routes)} routes to scrape.")
    ...
    # Filter out already scraped routes
