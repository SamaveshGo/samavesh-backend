...
INPUT_FILE = os.path.join(script_dir, "kdmt_lines_list.json")
OUTPUT_FILE = os.path.join(script_dir, "kalyan_dombivli_kdmt_bus_routes.json")
...
async def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: {INPUT_FILE} not found. Please run collect_kdmt_lines.py first.")
        sys.exit(1)
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        all_routes = json.load(f)
        
    # Filter to only keep KDMT agency lines (agency ID 888266)
    all_routes = [r for r in all_routes if "-888266-" in r["url"]]
    print(f"Filtered to KDMT agency lines only. Loaded {len(all_routes)} KDMT routes to scrape.")
