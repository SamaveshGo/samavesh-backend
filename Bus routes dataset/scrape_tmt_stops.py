...
INPUT_FILE = os.path.join(script_dir, "tmt_lines_list.json")
OUTPUT_FILE = os.path.join(script_dir, "thane_tmt_bus_routes.json")
...
async def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: {INPUT_FILE} not found. Please run collect_tmt_lines.py first.")
        sys.exit(1)
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        all_routes = json.load(f)
        
    # Filter to only keep TMT agency lines (agency ID 983983)
    all_routes = [r for r in all_routes if "-983983-" in r["url"]]
    print(f"Filtered to TMT agency lines only. Loaded {len(all_routes)} TMT routes to scrape.")
