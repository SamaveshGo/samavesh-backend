...
INPUT_FILE = os.path.join(script_dir, "vvmt_lines_list.json")
OUTPUT_FILE = os.path.join(script_dir, "vasai_virar_vvmt_bus_routes.json")
...
async def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: {INPUT_FILE} not found. Please run collect_vvmt_lines.py first.")
        sys.exit(1)
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        all_routes = json.load(f)
        
    # Filter to only keep VVMT agency lines (agency ID 888264)
    all_routes = [r for r in all_routes if "-888264-" in r["url"]]
    print(f"Filtered to VVMT agency lines only. Loaded {len(all_routes)} VVMT routes to scrape.")
