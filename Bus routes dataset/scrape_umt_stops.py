...
INPUT_FILE = os.path.join(script_dir, "umt_lines_list.json")
OUTPUT_FILE = os.path.join(script_dir, "ulhasnagar_umt_bus_routes.json")
...
async def main():
    if not os.path.exists(INPUT_FILE):
        print(f"Error: {INPUT_FILE} not found. Please run collect_umt_lines.py first.")
        sys.exit(1)
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        all_routes = json.load(f)
        
    # Filter to only keep UMT agency lines (agency ID 3775650)
    all_routes = [r for r in all_routes if "-3775650-" in r["url"]]
    print(f"Filtered to UMT agency lines only. Loaded {len(all_routes)} UMT routes to scrape.")
